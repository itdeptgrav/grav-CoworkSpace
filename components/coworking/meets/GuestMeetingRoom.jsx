        ...
        <>
            <div className="g-lk-hdr">
                ...
                <span className="g-lk-badge">
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#5BBF8A" }} />
                    LIVE
                </span>
                <span className="g-lk-guest">Joined as {guestName} · guest</span>
                <button className="g-peoplebtn" onClick={() => setShowPeople((v) => !v)} title="People in this meeting">
            ...
            {showPeople && (
                <div className="g-panel">
                    <div className="g-panel-hdr">
                        <span className="g-panel-title">People · {participants.length}</span>
                        <button className="g-panel-x" onClick={() => setShowPeople(false)}>×</button>
                    </div>
                        ...
                        {participants.map((p) => {
                            ...
                            return (
                                <div key={p.sid} className="g-prow">
                                    <div className="g-prow-av" style={{ background: avBg(name) }}>{initials(name)}</div>
                                    <span className="g-prow-name">
                                        {name} {p.isLocal && <span className="g-prow-you">(you)</span>}
                                    </span>
                                    <span style={{ color: micOn ? "#5BBF8A" : "#F87171", flexShrink: 0, display: "flex" }}>
                                        {micOn ? (
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function GuestMeetingRoom() {
    ...
    const handleDisconnected = useCallback(() => {
        try { recording.stopRecording?.(); } catch (_) { }
        setPhase("left");
    }, [recording]);
    ...
    const css = (
      ...
      /* ── Lobby ── */
      .g-lobby { display:flex; background:#111418; border:1px solid #22262C; border-radius:20px; overflow:hidden; width:min(880px, 100%); animation:g-fadein 0.3s ease; box-shadow:0 24px 70px rgba(0,0,0,0.5); }
      .g-lob-cam { flex:1.2; background:#000; position:relative; min-height:420px; display:flex; align-items:center; justify-content:center; }
      ...
      .g-lob-form { flex:1; padding:36px 34px; display:flex; flex-direction:column; gap:18px; box-sizing:border-box; }
      ...
      @media(max-width:760px){
        .g-lobby { flex-direction:column; }
        .g-lob-cam { min-height:240px; flex:none; }
        .g-lob-form { padding:24px 22px; }
    ...
    const Center = ({ children }) => (
        <div className="g-page">{css}{children}</div>
    );

    if (phase === "loading")
        return (
            <Center>
                <div style={{ width: 38, height: 38, border: "3px solid #22262C", borderTopColor: "#2563EB", borderRadius: "50%", animation: "g-spin 0.8s linear infinite", marginBottom: 16 }} />
                <p style={{ color: "#9AA0A6", fontSize: 13, margin: 0 }}>Loading meeting…</p>
            </Center>
        );
        ...
        return (
            <Center>
                <div className="g-card" style={{ textAlign: "center" }}>
                    ...
                    <p style={{ color: "#E8EAED", fontSize: 16, fontWeight: 600, margin: "0 0 8px" }}>Can't open this meeting</p>
                    <p style={{ color: "#9AA0A6", fontSize: 13, margin: 0, lineHeight: 1.6 }}>{error}</p>
                </div>
            </Center>
        );
    ...
    if (phase === "left")
        return (
            <Center>
                <div className="g-card" style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 30, marginBottom: 10 }}>👋</div>
                    <p style={{ color: "#E8EAED", fontSize: 16, fontWeight: 600, margin: "0 0 8px" }}>You left the meeting</p>
                    <p style={{ color: "#9AA0A6", fontSize: 13, margin: 0 }}>Thanks for joining, {guestName || "guest"}.</p>
                </div>
            </Center>
        );
    ...
    if (phase === "ended")
        return (
            <div className="g-page" style={{ justifyContent: "flex-start", overflowY: "auto" }}>
                {css}
                <div className="g-card" style={{ margin: "auto", maxHeight: "88vh", overflowY: "auto" }}>
                    <p style={{ color: "#E8EAED", fontSize: 17, fontWeight: 600, margin: "0 0 6px" }}>
                        {meetInfo?.meetTitle || "This meeting"} has ended
                    </p>
                    {!summary && (
                        <p style={{ color: "#9AA0A6", fontSize: 13, margin: 0, lineHeight: 1.6 }}>
                            The summary hasn't been generated yet — check back shortly using the same link.
                        </p>
                    )}
                    {summary && (
                        <div style={{ marginTop: 16 }}>
                            <h3 style={{ color: "#E8EAED", fontSize: 14, margin: "0 0 8px" }}>Summary</h3>
                            <p style={{ color: "#C9CDD3", fontSize: 13, lineHeight: 1.7, whiteSpace: "pre-wrap", margin: 0 }}>{summary.summary}</p>
                            {summary.actionItems?.length > 0 && (
                                <>
                                    <h3 style={{ color: "#E8EAED", fontSize: 14, margin: "18px 0 8px" }}>Action Items</h3>
                                    <ul style={{ color: "#C9CDD3", fontSize: 13, lineHeight: 1.8, margin: 0, paddingLeft: 18 }}>
                                        {summary.actionItems.map((a, i) => (
                                            <li key={i}>{typeof a === "string" ? a : a?.item || ""}</li>
                                        ))}
                                    </ul>
                                </>
                            )}
                ...
                <div className="g-lobby">
                    ...
                    <div className="g-lob-cam">
                        {camOn && !camErr ? (
                            ...
                            <div className="g-avatar-big" style={{ width: 96, height: 96, fontSize: 34, background: avBg(guestName || "G") }}>
                                {initials(guestName) || "?"}
                            </div>
                        )}
                        <span className="g-lob-cam-name">{guestName.trim() || "Guest"}</span>
                        <span className="g-lob-cam-state" style={{ color: camOn && !camErr ? "#5BBF8A" : "#F87171" }}>
                    ...
                    <div className="g-lob-form">
                        <div>
                            <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#2563EB", margin: "0 0 6px" }}>
                                You're invited
                            </p>
                            <h1 style={{ fontSize: 22, fontWeight: 700, color: "#E8EAED", margin: 0, lineHeight: 1.3 }}>
                                {meetInfo?.meetTitle || "CoWork Meeting"}
                            </h1>
                            <div className="g-chips" style={{ marginTop: 10 }}>
                                <span className="g-chip">
                                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#5BBF8A", display: "inline-block" }} />
                                    Live now
                                </span>
                        ...
                        <div>
                            <p className="g-lob-label" style={{ marginBottom: 7 }}>Your name</p>
                            <input
                                autoFocus
                                className="g-input"
                                value={guestName}
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            ...
                            <div className="g-devrow" onClick={() => setMicOn((v) => !v)}>
                                <span style={{ color: micOn ? "#5BBF8A" : "#F87171", display: "flex" }}>
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                        {micOn
                                            ? <><path d="M12 1a3 3 0 00-3 3v8a3 3 0 006 0V4a3 3 0 00-3-3z" /><path d="M19 10v2a7 7 0 01-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /></>
                                            : <><line x1="1" y1="1" x2="23" y2="23" /><path d="M9 9v3a3 3 0 005.12 2.12M15 9.34V4a3 3 0 00-5.94-.6" /><path d="M17 16.95A7 7 0 015 12v-2m14 0v2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /></>}
                                    </svg>
                                </span>
                                <div className="g-dev-name">
                                    <div className="g-dev-title">Microphone</div>
                                    <div className="g-dev-sub" style={{ color: micOn ? "#5BBF8A" : "#F87171" }}>{micOn ? "On" : "Off"}</div>
                                </div>
                                <div className="g-toggle" style={{ background: micOn ? "#2563EB" : "#3A3F47" }}>
                                    <div className="g-toggle-knob" style={{ transform: micOn ? "translateX(18px)" : "translateX(2px)" }} />
                                </div>
                            </div>
                            <div className="g-devrow" onClick={() => setCamOn((v) => !v)}>
                                <span style={{ color: camOn && !camErr ? "#5BBF8A" : "#F87171", display: "flex" }}>
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                        {camOn && !camErr
                                ...
                                <div className="g-dev-name">
                                    ...
                                    <div className="g-dev-sub" style={{ color: camOn && !camErr ? "#5BBF8A" : "#F87171" }}>
                                        {camErr ? "Unavailable" : camOn ? "On" : "Off"}
                                    </div>
                                </div>
                                <div className="g-toggle" style={{ background: camOn && !camErr ? "#2563EB" : "#3A3F47" }}>
                                    <div className="g-toggle-knob" style={{ transform: camOn && !camErr ? "translateX(18px)" : "translateX(2px)" }} />
                                </div>
                            </div>
                        </div>
                        ...
                        <button className="g-joinbtn" onClick={handleJoin} disabled={connecting || !guestName.trim()}>
                            {connecting ? "Joining…" : "Join Meeting"}
                        </button>

                        <p className="g-lob-foot">
                            By joining, your audio may be included in the meeting's AI-generated summary.
                        </p>
                    </div>
                </div>
            </Center>
    ...
    return (
        <div className="g-lk-wrap" data-lk-theme="default">
            ...
            >
                <RoomHeader meetTitle={meetInfo?.meetTitle || "CoWork Meeting"} guestName={guestName} />
