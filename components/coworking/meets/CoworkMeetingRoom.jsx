...
export default function CoworkMeetingRoom() {
    ...
    const handleEnd = async () => {
        ...
        try { await endMeeting(meetId); setToken(null); setPhase("ended"); }
        catch (e) { setError(e.message); }
    };
...
function LobbyScreen({ meet, info, isHost, busy, error, setError, employeeName, onStart, onJoinByCode, onBack }) {
    ...
    if (showPreJoin) {
        return (
            <div style={S.preJoinRoot}>
                <style>{CSS}</style>
                <div style={S.preJoinCard}>
                    <button onClick={() => setShowPreJoin(false)} style={S.backBtn}>← Back</button>
                    <h2 style={{ margin: "0 0 4px", fontSize: 18, fontWeight: 700, color: "#fff" }}>
                        {meet?.title || "CoWork Meeting"}
                    </h2>
                    <p style={{ margin: "0 0 20px", fontSize: 13, color: "#9AA0A6" }}>
                        Check your camera and microphone before joining
                    </p>
                    <PreJoin
                        onSubmit={handlePreJoinSubmit}
                        onError={(err) => setError(err.message)}
                        defaults={{ username: employeeName, videoEnabled: true, audioEnabled: true }}
                        joinLabel={isHost ? "Start Meeting" : "Join Meeting"}
                    />
                </div>
            </div>
        );
...
const S = {
    ...
    preJoinRoot: { position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", background: "#111", padding: 20, fontFamily: "'Google Sans','Roboto',sans-serif" },
    preJoinCard: { background: "#1E1E1E", borderRadius: 16, padding: "28px 32px", width: "100%", maxWidth: 600, boxShadow: "0 8px 32px rgba(0,0,0,0.5)", display: "flex", flexDirection: "column" },
};
