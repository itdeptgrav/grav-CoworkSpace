        ...
        <div style={s.wrapper}>
            ...
            <div style={s.inputRow}>
                ...
                <div style={s.toolBtns}>
                    ...
                    <ToolBtn onClick={() => pdfRef.current?.click()} disabled={disabled || recording || uploading} title="Attach PDF">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    ...
                    <ToolBtn onClick={recording ? stopRecording : startRecording} disabled={disabled || uploading} title={recording ? "Stop recording" : "Record voice"} active={recording}>
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                ...
                <button
                    onClick={handleSend}
                    disabled={!canSend}
                    style={{ ...s.sendBtn, opacity: canSend ? 1 : 0.4, cursor: canSend ? "pointer" : "not-allowed", background: canSend ? "var(--primary)" : "var(--gray-300)" }}
                    className="grav-btn"
                    title="Send (Enter)"
                >
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
...
function ToolBtn({ children, onClick, disabled, title, active }) {
    return (
        <button type="button" onClick={onClick} disabled={disabled} title={title}
            style={{ width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", background: active ? "var(--danger-light)" : "transparent", border: "none", borderRadius: "var(--radius-md)", cursor: disabled ? "not-allowed" : "pointer", color: active ? "var(--danger)" : "var(--gray-500)", opacity: disabled ? 0.45 : 1, flexShrink: 0, transition: "all var(--transition)" }}>
            {children}
        </button>
    );
}
...
const s = {
    wrapper: { display: "flex", flexDirection: "column", gap: 6, padding: "10px 14px 12px", background: "var(--surface)", borderTop: "1px solid var(--gray-200)", flexShrink: 0 },
    errorBar: { display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "var(--danger-light)", border: "1px solid var(--danger-mid, #F5C6C2)", borderRadius: "var(--radius-md)", fontSize: 12, color: "var(--danger)", fontFamily: "var(--font)" },
    errorIcon: { fontWeight: 700, flexShrink: 0, width: 16, textAlign: "center" },
    errorClose: { background: "none", border: "none", cursor: "pointer", color: "var(--danger)", fontSize: 13, padding: 0, lineHeight: 1, flexShrink: 0 },
    statusBar: { fontSize: 11, color: "var(--gray-500)", padding: "2px 4px", fontFamily: "var(--font)" },
    attRow: { display: "flex", flexWrap: "wrap", gap: 6 },
    attChip: { display: "flex", alignItems: "center", gap: 5, padding: "3px 8px", background: "var(--gray-50)", border: "1px solid var(--gray-200)", borderRadius: "var(--radius-full)", fontSize: 11, fontFamily: "var(--font)", color: "var(--gray-700)" },
    ...
    attRemove: { background: "none", border: "none", cursor: "pointer", color: "var(--gray-400)", fontSize: 11, padding: 0, lineHeight: 1 },
    inputRow: { display: "flex", alignItems: "flex-end", gap: 6 },
    toolBtns: { display: "flex", gap: 2, alignItems: "flex-end" },
    textarea: { flex: 1, padding: "8px 12px", border: "1.5px solid var(--gray-200)", borderRadius: "var(--radius-lg)", fontSize: 13, resize: "none", fontFamily: "var(--font)", lineHeight: 1.5, maxHeight: 96, overflowY: "auto", background: "var(--surface)", color: "var(--gray-800)", outline: "none", transition: "border-color 0.15s, box-shadow 0.15s", boxSizing: "border-box" },
    sendBtn: { width: 36, height: 36, borderRadius: "var(--radius-md)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all var(--transition)" },
};