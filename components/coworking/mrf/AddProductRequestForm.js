...
const emptyProduct = () => ({ itemName:"", category:"", unit:"", notes:"", attributes:[] })
...
export default function AddProductRequestForm({
  ...
  const removeAttrValue = (pIdx, aIdx, vIdx) => setProducts(p => p.map((row,i) => {
    if (i!==pIdx) return row
    return { ...row, attributes: row.attributes.map((a,j) => {
      ...
      {products.map((p, pIdx) => (
        <div key={pIdx} style={{ border:`1px solid ${C.border}`, borderRadius:6, padding:"12px", background:C.surface }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
            <span style={{ fontSize:11, fontWeight:700, color:C.textSub, textTransform:"uppercase", letterSpacing:"0.05em" }}>Product {pIdx+1}</span>
            {products.length > 1 && (
