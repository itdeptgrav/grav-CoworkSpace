...
export default function DeviceDetailPage({ params }) {
    ...
    // fetch device
    useEffect(() => {
        api.get(`/devices/${id}`).then(setDevice).catch(() => { })
    }, [id])
    ...
    useEffect(() => {
        ...
        socket.on(`activity-update-${id}`, (act) => {
            if (act.date === selectedDate) {
                ...
                setTotalTime(prev => prev + (act.durationSec || 0))
            }
        })

        // system events
        socket.on(`system-event-${id}`, (evt) => {
            if (evt.date === selectedDate) setSystemEvents(prev => [evt, ...prev])
        })

        // device update
        socket.on('device-update', (dev) => {
    ...
    useEffect(() => {
        if (!screenshotMode || screenshotPaused) {
    ...
    const buildTimeline = () => {
        const allEvents = [
            ...activity.map(a => ({ type: 'activity', time: new Date(a.startTime || 0), data: a })),
            ...systemEvents.map(s => ({ type: 'system', time: new Date(s.timestamp || 0), data: s }))
        ...
        for (const evt of allEvents) {
            ...
            } else if (evt.type === 'system' && ['SYSTEM_SHUTDOWN', 'SYSTEM_RESTART', 'LAPTOP_SLEEP'].includes(evt.data.event)) {
                if (current) { current.logoutTime = evt.time; current.logoutType = evt.data.event }
                current = null
            } else if (evt.type === 'activity') {
                if (!current) { current = { loginTime: evt.time, activities: [], logoutTime: null, logoutType: null, implicit: true }; sessions.push(current) }
            ...
            const children = [
                ...
                new Table({
                    ...
                    rows: [
                        ...
                        ...sortedApps.map(app => {
                            const isAlert = ['Video', 'Entertainment', 'Social Media', 'Shopping', '⚠️ Terminal', '⚠️ System Tool', '⚠️ Virtual Machine', '⚠️ VPN', '⚠️ Remote Desktop', '⚠️ Screen Recorder'].includes(app.category)
                            const rowFill = isAlert ? 'FDECEA' : 'FFFFFF'
                            const textColor = isAlert ? 'C0392B' : '111827'
                            return new TableRow({
                                children: [
                                    new TableCell({ borders, width: { size: 4000, type: WidthType.DXA }, shading: { fill: rowFill, type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [new TextRun({ text: `${isAlert ? '⚠ ' : ''}${app.siteLabel}`, size: 20, font: 'Arial', color: textColor, bold: isAlert })] })] }),
                                    new TableCell({ borders, width: { size: 2680, type: WidthType.DXA }, shading: { fill: rowFill, type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [new TextRun({ text: fmt(app.totalSec), size: 20, font: 'Arial', color: textColor, bold: isAlert })] })] }),
                                    new TableCell({ borders, width: { size: 2680, type: WidthType.DXA }, shading: { fill: rowFill, type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 120, right: 120 }, children: [new Paragraph({ children: [new TextRun({ text: String(app.count), size: 20, font: 'Arial', color: textColor, bold: isAlert })] })] }),
                                ]
                            })
                        }),
                    ],
                ...
                <div style={{ background: "#fff", border: `1px solid ${currentApp?.isIdle ? "#FDE68A" : "#A7F3D0"}`, borderLeft: `4px solid ${currentApp?.isIdle ? "#D97706" : "#10B981"}`, borderRadius: 10, padding: "16px 20px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                    <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 11, fontWeight: 700, color: currentApp.isIdle ? "#D97706" : "#059669", letterSpacing: "0.08em" }}>{currentApp.isIdle ? "⏸ IDLE" : "LIVE NOW"}</span>
                        </div>
                        ...
                        <div key={si} style={{ borderBottom: "2px solid #E5E7EB", marginBottom: 4 }}>
                            ...
                            {totalView && Object.values(
                                session.activities.reduce((acc, row) => {
                                    ...
                                    acc[key].byTitle[title].totalSec += row.durationSec || 0
                                    acc[key].byTitle[title].count += 1
                                    return acc
                                }, {})
                            ).sort((a, b) => b.totalSec - a.totalSec).map(app => (
                                <div key={app.siteLabel} style={{ borderBottom: "1px solid #F3F4F6" }}>
                    ...
                    {timeline.map((session, si) => (
                        <div key={si} style={{ borderBottom: "2px solid #E5E7EB", marginBottom: 4 }}>
                            ...
                            {session.activities.map((act, ai) => {
                                ...
                                return (
                                    <div key={ai} style={{ display: "flex", alignItems: "center", gap: 12, padding: "7px 16px 7px 44px", borderBottom: "1px solid #F9FAFB" }}>
                                        <div style={{ width: 6, height: 6, borderRadius: "50%", background: color, flexShrink: 0 }} />
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <span style={{ fontSize: 12, fontWeight: 600, color: "#111827" }}>{act.siteLabel}</span>
                ...
                <div style={tableWrap}>
                    ...
                    <div style={{ overflowX: "auto" }}>
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            ...
                            <tbody>
                                {activity.map((row, i) => (
                                    <tr key={row._id || i} style={{ borderBottom: "1px solid #F3F4F6" }}>
