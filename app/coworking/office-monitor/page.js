...
function LiveActivityBadge({ machineId }) {
    ...
    useEffect(() => {
        // fetch initial
        api.get(`/activity/current/${machineId}`).then(data => {
            if (data?.isActive) {
                const diff = Date.now() - new Date(data.updatedAt).getTime()
                if (diff < 15000) setCurr(data)
            }
        }).catch(() => { })

        // socket
        const socket = io(BACKEND)
        socket.on(`current-activity-${machineId}`, (data) => {
            if (data?.isActive) {
                const diff = Date.now() - new Date(data.updatedAt).getTime()
                setCurr(diff < 15000 ? data : null)
            } else setCurr(null)
        })
    ...
    return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "2px 9px", borderRadius: 20, background: color + "15", border: `1px solid ${color}30`, fontSize: 11, fontWeight: 600, color, marginTop: 4 }}>
...
export default function OfficeMonitorPage() {
    ...
    // socket for real-time device updates
    useEffect(() => {
        const socket = io(BACKEND)
        socket.on('device-update', (device) => {
            setDevices(prev => {
                const idx = prev.findIndex(d => d.machineId === device.machineId)
                if (idx >= 0) {
                    const updated = [...prev]
                    updated[idx] = device
    ...
    useEffect(() => {
        api.get('/agent-version/deployed').then(data => setDeployedVersion(data)).catch(() => { })
    }, [])

    const activateDevice = (id) => api.patch(`/devices/${id}`, { status: 'active', tracking: true, agentStatus: 'running' })
    const deactivateDevice = (id) => api.patch(`/devices/${id}`, { status: 'inactive', tracking: false })
    const toggleTracking = (id, cur) => api.patch(`/devices/${id}`, { tracking: !cur })
    const renameDevice = async (id) => {
        if (!newName.trim()) return
        await api.patch(`/devices/${id}`, { customName: newName.trim() })
        setRenaming(null); setNewName("")
    }

    const handlePushToUsers = async () => {
        setReleasing(true)
        await api.patch('/agent-version/release', {})
        const updated = await api.get('/agent-version/deployed')
        setDeployedVersion(updated)
        setReleasing(false)
    }
            ...
            <div style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
                    ...
                    <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        <button onClick={() => setShowSettings(prev => !prev)} style={{ padding: "6px 14px", background: showSettings ? "#EEF2FF" : "#F9FAFB", color: showSettings ? "#6366F1" : "#374151", border: `1px solid ${showSettings ? "#C7D2FE" : "#E5E7EB"}`, borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                            ⚙ Settings
                        </button>
                    </div>
                </div>
                    ...
                    {pending.map(d => (
                        <div key={d.machineId} style={{ ...card, borderLeft: "3px solid #F59E0B" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 14, flex: 1 }}>
                                <div style={{ ...avatar, background: "#FEF3C7", color: "#D97706" }}>{(d.customName || d.machineId)[0].toUpperCase()}</div>
                                <div>
                                    <div style={cardName}>{d.customName || d.machineId}</div>
                                    <div style={cardMeta}>{d.machineId} · {d.ipAddress} · Installed {d.installedAt ? new Date(d.installedAt).toLocaleDateString() : ''}</div>
                                </div>
                            </div>
            ...
            {active.length > 0 && (
                <div style={{ marginBottom: 24 }}>
                    ...
                    {active.map(d => {
                        ...
                        return (
                            <div key={d.machineId} style={{ ...card, borderLeft: `3px solid ${status === "online" ? "#10B981" : status === "warning" ? "#F59E0B" : "#EF4444"}` }}>
                                <div style={{ display: "flex", alignItems: "flex-start", gap: 14, flex: 1, minWidth: 0 }}>
                                    <div style={{ ...avatar, background: status === "online" ? "#D1FAE5" : "#F3F4F6", color: status === "online" ? "#059669" : "#6B7280" }}>
                                        {(d.customName || d.machineId)[0].toUpperCase()}
                                    </div>
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        {renaming === d.machineId ? (
                                            <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 4 }}>
                                                <input value={newName} autoFocus onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === "Enter" && renameDevice(d.machineId)} placeholder="Enter name" style={{ padding: "5px 10px", border: "1.5px solid #2563EB", borderRadius: 6, fontSize: 13, outline: "none", width: 160, color: "#111827" }} />
                                            ...
                                            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2, flexWrap: "wrap" }}>
                                                ...
                                                <button style={{ background: "none", border: "none", color: "#9CA3AF", fontSize: 13, cursor: "pointer", padding: "0 2px" }} onClick={() => { setRenaming(d.machineId); setNewName(d.customName || "") }}>✎</button>
                                        ...
                                        <div style={cardMeta}>
                                            {d.machineId} · {d.ipAddress} · Last seen {d.lastSeen ? new Date(d.lastSeen).toLocaleTimeString() : '—'}
                                            {d.lastLoginTime && ` · Login ${new Date(d.lastLoginTime).toLocaleTimeString()}`}
                                        </div>
                                        ...
                                        {(status === "stopped" || status === "offline") && d.stopReason && (
                                            <div style={{ marginTop: 4, padding: "3px 8px", background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 5, fontSize: 11, color: "#B91C1C", display: "inline-block" }}>
                                                ...
                                                {d.stoppedAt && ` at ${new Date(d.stoppedAt).toLocaleTimeString()}`}
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, flexShrink: 0 }}>
                                    ...
                                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
                                        <button style={d.tracking ? btnWarning : btnSuccess} onClick={() => toggleTracking(d.machineId, d.tracking)}>{d.tracking ? "Pause" : "Resume"}</button>
                                        <button style={btnBlue} onClick={() => router.push(`/coworking/office-monitor/${d.machineId}`)}>View Activity →</button>
                ...
                <div style={{ marginBottom: 24 }}>
                    ...
                    {inactive.map(d => (
                        <div key={d.machineId} style={{ ...card, opacity: 0.6 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 14, flex: 1 }}>
                                <div style={{ ...avatar, background: "#F3F4F6", color: "#9CA3AF" }}>{(d.customName || d.machineId)[0].toUpperCase()}</div>
                                <div>
                                    <div style={cardName}>{d.customName || d.machineId}</div>
                ...
                <div style={{ marginBottom: 24 }}>
                    ...
                    <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 10, padding: "20px 24px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                        {deployedVersion && (
                            <div style={{ marginBottom: 16, padding: "10px 14px", background: deployedVersion.status === "released" ? "#ECFDF5" : "#FFFBEB", border: `1px solid ${deployedVersion.status === "released" ? "#A7F3D0" : "#FDE68A"}`, borderRadius: 8, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                                ...
                                {deployedVersion.releasedAt && (
                                    <span style={{ fontSize: 11, color: "#9CA3AF" }}>Released {new Date(deployedVersion.releasedAt).toLocaleString()}</span>
                                )}
                            </div>
                        )}
                        ...
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                            <button onClick={handlePushToUsers} disabled={!deployedVersion || deployedVersion.status === "released" || releasing}
                        ...
                        <div style={{ marginTop: 14, fontSize: 11, color: "#9CA3AF", lineHeight: 1.6 }}>
                            <strong style={{ color: "#6B7280" }}>Workflow:</strong> 1. Build new exe → 2. Run <code style={{ background: "#F3F4F6", padding: "1px 4px", borderRadius: 3 }}>node push-update.js 1.1</code> → 3. Click Push to User Laptops
                        </div>
                    </div>
                </div>
            )}
...
const card = { background: "#fff", border: "1px solid #E5E7EB", borderRadius: 10, padding: "14px 18px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }
const avatar = { width: 40, height: 40, borderRadius: 10, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 15 }
