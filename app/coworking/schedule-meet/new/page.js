...
export default function NewMeetPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.dateTime) { setError("Date and time is required."); return; }
    if (participantIds.length === 0) { setError("Add at least one participant."); return; }
  ...
  const selectedEmps = employees.filter(e => participantIds.includes(e.employeeId));
    ...
    <>
      ...
      <div className="sm-page">
        ...
        <div className="sm-topbar">
          <button className="sm-topbar-back" onClick={() => router.push("/coworking/schedule-meet")} title="Back">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M19 12H5M12 5l-7 7 7 7" />
            </svg>
          </button>
          <span className="sm-topbar-title">New Meeting</span>
          <button className="sm-topbar-submit" onClick={handleSubmit} disabled={submitting}>
            {submitting ? (
              <>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" style={{ animation: "sm-spin 1s linear infinite" }}><path d="M21 12a9 9 0 11-6.219-8.56" /></svg>
                Scheduling…
              </>
            ) : "Schedule Meeting"}
          </button>
        ...
        <form onSubmit={handleSubmit}>
          <div className="sm-body">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div className="sm-card">
                <div className="sm-card-head">
                  ...
                  <div>
                    ...
                    <div className="sm-card-sub">Basic information about the meeting</div>
                  </div>
                </div>
                <div className="sm-card-body">
                  <div className="sm-field">
                    <label className="sm-label">Title <span className="sm-req">*</span></label>
                    <input className="sm-input" value={form.title}
                  ...
                  <div className="sm-field">
                    ...
                    <textarea className="sm-textarea" value={form.description}
                  ...
                  <div className="sm-row-2">
                    ...
                    <div className="sm-field">
                      <label className="sm-label">Meeting Link <span style={{ fontWeight: 400, textTransform: "none", fontSize: 11, color: "#9aa0a6", marginLeft: 4 }}>(optional)</span></label>
                      <input className="sm-input" value={form.googleMeetLink}
              ...
              <div className="sm-card">
                <div className="sm-card-head">
                  <div className="sm-card-icon" style={{ background: "#e6f4ea" }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#1e8e3e" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <div>
                    <div className="sm-card-title">
                      ...
                      {participantIds.length > 0 && (
                        <span style={{ fontSize: 12, fontWeight: 600, background: "#e8f0fe", color: "#1a73e8", borderRadius: 99, padding: "2px 8px", marginLeft: 8 }}>
                ...
                <div className="sm-card-body">
                  ...
                  {allDepts.length > 0 && (
                    <div className="sm-dept-section">
                      <div className="sm-dept-header">
                        <span className="sm-dept-header-label">Filter by department</span>
                        {selectedDepts.length > 0 && (
                          <button type="button" className="sm-dept-clear" onClick={() => setSelectedDepts([])}>
                            Clear filter
                          </button>
                        )}
                      </div>
                  ...
                  <div className="sm-select-bar">
                    ...
                    {visibleEmployees.length > 0 && (
                      <button type="button" className="sm-select-all-btn" onClick={selectAllVisible}>
                        Select all
                      </button>
                    )}
                  </div>
                  ...
                  <div className="sm-emp-grid">
                    {employees.length === 0 ? (
                      <span style={{ fontSize: 13, color: "#9aa0a6" }}>Loading employees…</span>
                    ) : visibleEmployees.length === 0 ? (
                      <span style={{ fontSize: 13, color: "#9aa0a6" }}>No employees in selected department(s).</span>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div className="sm-card-head">
                  <div>
                    ...
                    <div className="sm-card-sub">
                      {participantIds.length === 0 ? "No one selected yet" : `${participantIds.length} invited`}
                    </div>
                  </div>
                  {participantIds.length > 0 && (
                    <button type="button" onClick={() => setParticipantIds([])}
                      style={{ marginLeft: "auto", fontSize: 12, color: "#d93025", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 500 }}>
                      Clear all
                    </button>
                  )}
                </div>
                ...
                {participantIds.length === 0 ? (
                  <div className="sm-preview-empty">
                    <div className="sm-preview-empty-icon">
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#bdc1c6" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" /><circle cx="9" cy="7" r="4" />
                        <path d="M23 21v-2a4 4 0 00-3-3.87" /><path d="M16 3.13a4 4 0 010 7.75" />
                      </svg>
                    </div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: "#80868b", marginBottom: 4 }}>No participants yet</div>
                    <div style={{ fontSize: 12, color: "#bdc1c6" }}>Select employees from the left panel</div>
                  </div>
                  ...
                  <div key={emp.employeeId} className="sm-participant-row">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div className="sm-p-dept">
                        {emp.department || ""}
                        {emp.role === "tl" ? (emp.department ? " · TL" : "TL") : ""}
                      </div>
                    </div>
                    <button type="button" className="sm-p-remove"
                      onClick={() => toggleParticipant(emp.employeeId)} title="Remove">
              ...
              {(form.title || form.dateTime || participantIds.length > 0) && (
                <div className="sm-summary">
                  <div style={{ fontSize: 12, fontWeight: 600, color: "#5f6368", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>
                    Summary
                  </div>
                  {form.title && (
                    <div className="sm-summary-row">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1a73e8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                        <polygon points="23 7 16 12 23 17 23 7" /><rect x="1" y="5" width="15" height="14" rx="2" />
                      </svg>
                  ...
                  {form.dateTime && (
                    <div className="sm-summary-row">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#5f6368" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                        <rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" />
                      </svg>
                      {new Date(form.dateTime).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}
                    </div>
                  )}
                  {participantIds.length > 0 && (
                    <div className="sm-summary-row">
                      ...
                      {participantIds.length} participant{participantIds.length !== 1 ? "s" : ""} invited
                    </div>
                  )}
                </div>
              )}
