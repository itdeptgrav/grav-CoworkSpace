...
// ── Gmail: per-user (shows emails for the logged-in employee's email) ──────────
export const getMyGmailInbox = (email, max = 30) => gFetch(`/api/google/gmail/my-inbox?email=${encodeURIComponent(email)}&max=${max}`);
export const getAllGmailInbox = (max = 30) => gFetch(`/api/google/gmail/all-inbox?max=${max}`);