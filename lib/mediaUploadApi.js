...
// ── Google Drive DIRECT upload (resumable) ───────────────────────
// Backend creates a session (tiny) → browser PUTs bytes straight
// to Google (backend never touches them) → backend finalizes (tiny).
    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.onload = () => {
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                catch { reject(new Error("Drive returned unreadable response")); }
            } else {
...
async function uploadToDrive(file, onProgress) {
    const { sessionUrl } = await api("/cowork/upload/drive-session", {
        method: "POST",
        body: JSON.stringify({
    ...
    if (!uploaded?.id) throw new Error("Drive upload finished without a file ID");
    return api("/cowork/upload/drive-finalize", {
        method: "POST",
        body: JSON.stringify({ fileId: uploaded.id }),
    });
}

// Images → Google Drive (rendered via lh3.googleusercontent.com)
// Return shape kept identical to old Cloudinary version — no caller
// (DM paste, group chat, tasks) needs any change.
export async function uploadImage(file, folder = "cowork-messages", onProgress) {
    const d = await uploadToDrive(file, onProgress);
    return {
        url: d.url,                                   // try this first
        proxyUrl: `${BASE}/cowork/media/view/${d.fileId}`, // onError fallback — always works
        publicId: d.fileId,
        format: (d.mimeType || "").split("/")[1] || "",
        bytes: Number(d.size) || file.size,
...
export async function uploadPDF(file, onProgress) {
    // Same direct-to-Drive pipeline as images — a 200MB PDF no longer
    // touches the backend either.
...
export async function listTasks(cursor = null, pageSize = 100) {
    const qs = `?pageSize=${pageSize}${cursor ? `&cursor=${cursor}` : ""}`;
    try {
        const d = await api(`/cowork/task/list-hierarchy${qs}`);
        return { tasks: d.tasks || [], nextCursor: d.nextCursor ?? null, hasMore: !!d.hasMore };
    } catch {
        const d = await api(`/cowork/task/list${qs}`);
        return { tasks: d.tasks || [], nextCursor: null, hasMore: false };
    }
}
...
export async function getForwardBudget(taskId) {
    return api(`/cowork/task/${taskId}/forward-budget`);
}
...
export async function moveTaskToFolder(taskId, folderId) {
    return api(`/cowork/task/${taskId}/move-to-folder`, {
        method: "POST",
        body: JSON.stringify({ folderId }),
    });
}

export async function editTaskDetails(taskId, { title, description, requirements }) {
    return api(`/cowork/task/${taskId}/edit-details`, {
        method: "PATCH",
        body: JSON.stringify({ title, description, requirements }),
    });
}

export async function resetTaskToDraft(taskId) {
    return api(`/cowork/task/${taskId}/reset-to-draft`, { method: "POST" });
}
