...
// Tasks
export async function createTask({ title, description, notes, assigneeIds, dueDate, priority, parentTaskId, createdByRole, createdBy, createdByCeo, createdByTl, status, isFolder, isRepeat, repeatConfig, isThirdParty, thirdPartyConfig, isGoal, goalConfig, hasTimer, fixedDeadline, isSelfAssigned, visibleTo, approverId, approverName, senderTimerWindowSecs, isGoldTask = false, c2Config = null }) {
    const d = await api("/cowork/task/create", {
        method: "POST",
        body: JSON.stringify({
            title, description, notes, assigneeIds, dueDate, priority,
