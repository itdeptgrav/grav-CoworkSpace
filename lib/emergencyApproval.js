...
export async function requestEmergencyApproval(employeeId, employeeName, gapMs, reason) {
    ...
    // Who is asking? A TL's (or CEO's) emergency request must NOT be routed down
    // their HR reporting line — another TL can't sign off on a TL. It escalates
    // to the CEO/admin. cowork_employees doc id === employeeId.
    let requesterRole = "employee";
    try {
        const empSnap = await getDoc(doc(firebaseDb, "cowork_employees", employeeId));
        if (empSnap.exists()) requesterRole = empSnap.data().role || "employee";
    } catch (e) {
        console.warn("[emergency-approval] role lookup failed:", e.message);
    }
    const escalateToCeo = requesterRole === "tl" || requesterRole === "ceo";

    let tlId = null, tlName = null;
    if (!escalateToCeo) try {
    ...
    return { requested: true, tlFound: !!tlId, escalateToCeo };
}