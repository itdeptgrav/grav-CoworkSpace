...
export const taskForwardApi = {
    ...
    // extensionSecs: the exact typed delta in seconds. Passed alongside proposedDate so the
    // backend can use it directly (avoids shrinkage when approval happens minutes after request).
    proposeDeadline: (taskId, proposedDate, workedSecs = 0, extensionSecs = 0) =>
        coworkFetch(`/task/${taskId}/propose-deadline`, {
            method: "POST",
            body: JSON.stringify({ proposedDate, workedSecs, extensionSecs }),
        }),
