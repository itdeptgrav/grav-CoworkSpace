...
// These follow the same route as an MRF: the requester's Primary Manager/TL
// approves first, and only then does the Store see it to match or register.
export const listProductRequests = () => request("/product-requests");
export const createProductRequest = (payload) =>
  request("/product-requests", { method: "POST", body: payload });
